...
interface GceAlResultModalProps {
  isOpen: boolean;
  onClose: () => void;
  initialData: string;
...
const SUBJECT_CATEGORIES = [
  {
    category: "Advanced Level - Mathematics / Alt. Subjects",
    subjects: [
      "Combined Maths",
      "Physics",
      "Chemistry",
  ...
  {
    category: "Advanced Level - Commerce",
    subjects: [
      "Accounting",
      "Economics",
      "Business Studies"
...
export function GceAlResultModal({ isOpen, onClose, initialData, onSave, readOnly = false }: GceAlResultModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-slate-100 shrink-0">
          <div>
            <h2 className="text-xl font-bold text-slate-800">GCE A/L Exam Results</h2>
          ...
          <div className="space-y-8">
            {SUBJECT_CATEGORIES.map((cat) => (
              <div key={cat.category} className="space-y-4">
                <h3 className="text-sm font-black text-indigo-600 uppercase tracking-wider border-b border-indigo-100 pb-2">
                  {cat.category}
                </h3>
                  ...
                  {cat.subjects.map(subject => (
                    <div key={subject} className="flex items-center justify-between gap-4 p-3 rounded-xl bg-slate-50 border border-slate-100">
                      ...
                      <select
                        ...
                        disabled={readOnly}
                        className="h-8 w-20 px-2 rounded-lg border border-slate-200 text-xs font-bold text-slate-700 focus:ring-2 focus:ring-indigo-500/20 disabled:opacity-50 disabled:cursor-not-allowed bg-white"
                      >
                        {GRADES.map(g => (
        ...
        <div className="p-4 sm:p-6 border-t border-slate-100 shrink-0 flex justify-end gap-3 bg-slate-50/50">
          ...
          {!readOnly && (
            <Button type="button" onClick={handleSave} className="h-10 px-6 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs shadow-md shadow-indigo-600/20 transition-all">
